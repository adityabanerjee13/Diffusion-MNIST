...
class DownBlock(nn.Module):
    def __init__(self, in_channels, out_channels, t_emb_dim, down_sample, num_heads, layers):
        super().__init__()
        self.layers = layers
        ...
        self.down_sample_conv = [
            nn.Conv2d(out_channels, out_channels, kernel_size=4,
                                             stride=2, padding=1) if self.down_sample else nn.Identity()
            for _ in range(layers)
            ]
        self.out_downblock  = nn.Conv2d(out_channels*layers, out_channels, kernel_size=5, padding=2)
...
class UpBlock(nn.Module):
    def __init__(self, in_channels, out_channels, t_emb_dim, up_sample, num_heads, layers):
        super().__init__()
    ...
    def __init__(self, model_config):
        ...
        for i in range(len(self.down_channels) - 1):
            self.downs.append(DownBlock(self.down_channels[i], self.down_channels[i+1], self.t_emb_dim,
                                       down_sample=self.down_sample[i], num_heads=num_heads, layers=layers))
